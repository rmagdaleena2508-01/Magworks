...
- **A gentle intro instead of a hard cut.** *Why:* jumping straight to the hero image the moment the page opened felt abrupt. *How:* the page opens on a plain black screen. A first line, in the site's serif font, **types itself out** — "Welcome to my corner of the internet" — then fades and lifts up out of the way. The first line types in two beats: it writes "Welcome", pauses, then finishes "…to my corner of the internet!". A second line rises up from the bottom into the same spot with a short, friendly note. Then, instead of black, a sheet of **frosted glass** appears over the hero: you can see the hero the whole time, just **blurred, as if through pixel-shaped glass tiles**. The glass then clears in tiles **from the top down**, and where it clears the hero turns sharp — so the sharp image is uncovered progressively, never shown all at once and cut. *How it works — two stacked layers over the live hero:* (1) a **`backdrop-filter: blur()`** sheet that frosts the hero, uncovered top-to-bottom by a tiny per-frame **alpha mask** (a small canvas, a few pixels per tile); and (2) a **drawn glass-tile canvas** — translucent white squares with seams and a light top-edge highlight — that clears on the same schedule. The drawn tiles make the "glass squares" clearly visible on **every device, including mobile Safari**, where masking a `backdrop-filter` can silently fail (that was why the effect looked invisible on phones). Both clear on a **GSAP** `sine.inOut` tween over ~3s and end fully clear, so removing the layers is invisible — no sudden cut. Only a stray *key* skips it now; a tap or scroll no longer nukes the intro on mobile (the on-screen "skip intro" button still does). We use `backdrop-filter` on the live hero rather than a WebGL shader because the hero is real DOM (including a video), which a shader can't cheaply sample every frame. The intro plays on every load/reload (not once per session), and any key, tap, or the "skip intro" button jumps straight to the site. There is a small "skip intro" button, any key/tap/scroll skips it, it plays only once per visit (so reloads don't repeat it), and it is turned off entirely for people who ask for less motion. *Idea we followed:* ease people in, but never trap them behind it.
...
| Intro animation | **GSAP** + **TextPlugin** (CDN) for the typed/slide lines, then a frosted-glass reveal: a `backdrop-filter: blur()` layer (canvas alpha-mask, top-to-bottom) **plus a drawn translucent glass-tile canvas** so the pixel tiles are visible on every device incl. mobile Safari; slow `sine.inOut` (~3s), plays every load, key/button skip, off for reduced-motion |
