...
| Intro animation | **GSAP** + **TextPlugin** (CDN) for the two-beat typed line and slide-in line, then a frosted-glass reveal — one `backdrop-filter: blur()` layer over the hero, uncovered top-to-bottom in pixel tiles by a tiny per-frame canvas **alpha mask** on a slow `sine.inOut` (~3s); plays every load, skippable, off for reduced-motion |
