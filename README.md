...
| Intro animation | **GSAP** + **TextPlugin** (CDN) for the two-beat typed line and slide-in line, then a **Canvas 2D** frosted-glass pixel dissolve — per-block alpha fade + easing blur on a slow `sine.inOut` (~2.9s) — to uncover the hero; plays every load, skippable, off for reduced-motion |
