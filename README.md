...
- **The menu knows where you are (sliding underline).** *Why:* it helps you keep track of which part you are reading, and a line that slides feels smoother than one that blinks on and off. *How:* there is just **one** small blue line under the menu. As a section scrolls near the top of the screen, the line glides sideways to sit under that section's name (it changes its width and slides across in about a third of a second, instead of jumping). The menu order also follows the page order (Home, Contact, Projects, Stack, Writing), so the line always moves in the same direction you are scrolling. On the Contact and Writing pages there is nothing to scroll-track, so the line rests under the page you are on and glides to any menu item you hover, then slides back when you move away. *Idea we followed:* the page should quietly point at itself, with soft motion instead of a snap.
...
| Scroll progress + active nav | `scroll` listener driving a `scaleX` bar + an **IntersectionObserver** that drives one sliding underline (`width` + `translateX` transition) across the menu; nav order matches the page's section order. On contact/writing the underline sits under the current page and follows hover |
