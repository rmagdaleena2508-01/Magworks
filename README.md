...
- **A gentle intro instead of a hard cut.** *Why:* jumping straight to the hero image the moment the page opened felt abrupt. *How:* the page opens on a plain black screen. A first line, in the site's serif font, **types itself out** — "Welcome to my corner of the internet" — then fades and lifts up out of the way. The first line is typed one character at a time at a **human pace**: it writes "Welcome", pauses, then finishes "…to my corner of the internet!". (The typing grows the text with `slice()` on a character index rather than a plugin that re-sets the node, so there is no blink between the two beats — it was blinking on mobile before.) A second line rises up from the bottom into the same spot with a short, friendly note. Then a **pixelated glass reveal** uncovers the hero top-to-bottom: two layers sit over the hero — a `backdrop-filter: blur()` sheet (real blur where supported) and a **drawn smoked-glass tile canvas** (translucent squares with seams and a top highlight) that also dims the hero, so the glass tiles are clearly visible and the hero is never shown sharp-and-whole, on **every device including mobile**. The tiles clear top-first, each fading over a soft window, on a slow **GSAP** `sine.inOut` tween (~2.9s, easy on the eyes). *Order fix:* the covering frame is drawn **synchronously before** the black backdrop is handed off, so the hero can never flash for a frame before the glass is in place (that stray blink was happening on mobile). It ends fully clear, so removing the layers is invisible — no sudden cut. Only a stray *key* auto-skips it; a tap or scroll no longer nukes the intro on mobile (the on-screen "skip intro" button still does). The intro plays on every load/reload (not once per session), and any key, tap, or the "skip intro" button jumps straight to the site. There is a small "skip intro" button, any key/tap/scroll skips it, it plays only once per visit (so reloads don't repeat it), and it is turned off entirely for people who ask for less motion. *Idea we followed:* ease people in, but never trap them behind it.
...
| Intro animation | **GSAP** (CDN) — character-index typewriter (human pace, no blink) for the two-beat line, slide-in second line, then a pixelated **glass** reveal — `backdrop-filter: blur()` + a drawn smoked-glass tile canvas, cleared top-to-bottom (per-tile soft fade, slow `sine.inOut` ~2.9s); covering frame pre-drawn to avoid a hero flash; works on mobile; plays every load, key/button skip, off for reduced-motion |
