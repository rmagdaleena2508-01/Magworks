...
| Intro animation | **GSAP** (CDN) — character-index typewriter (human pace, no blink) for the two-beat line, slide-in second line, then a **Canvas 2D** black pixelated top-to-bottom dissolve (per-block soft fade, slow `sine.inOut` ~2.9s); plays every load, key/button skip, off for reduced-motion |
