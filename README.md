...
- **A gentle intro instead of a hard cut.** *Why:* jumping straight to the hero image the moment the page opened felt abrupt. *How:* the page opens on a plain black screen. A first line, in the site's serif font, **types itself out** — "Welcome to my corner of the internet" — then fades and lifts up out of the way. A second line rises up from the bottom into the same spot with a short, friendly note (ending "…no AI btw"). Then, instead of a plain fade, the black screen **breaks into pixel blocks that clear from the top down** — a pixelated dissolve — until the whole hero is revealed. It is built with the **GSAP** animation library (its TextPlugin does the typing and its tween smoothly drives the dissolve) plus a small **HTML canvas** that paints and clears the pixel blocks. We chose a canvas mosaic over a WebGL shader because the intro only needs to dissolve a solid black layer to show the hero underneath, so a tiny canvas does it at a smooth 60fps with no extra library. There is a small "skip intro" button, any key/tap/scroll skips it, it plays only once per visit (so reloads don't repeat it), and it is turned off entirely for people who ask for less motion. *Idea we followed:* ease people in, but never trap them behind it.
...
| Intro animation | **GSAP** + **TextPlugin** (CDN) for the typed/slide lines, then a **Canvas 2D** pixelated top-to-bottom dissolve (GSAP-tweened block reveal) to uncover the hero; session-once, skippable, off for reduced-motion |
